...
Command the actuator (keep publishing to hold motor on):
  ros2 topic pub /bucket/lift/lift_driver/command std_msgs/msg/Float64 "data: 1.0" --rate 5
  ros2 topic pub /bucket/lift/lift_driver/command std_msgs/msg/Float64 "data: -1.0" --rate 5
  ros2 topic pub /bucket/lift/lift_driver/command std_msgs/msg/Float64 "data: 0.0" --rate 5

  (substitute tilt_driver when which:=tilt)
"""
...
def _launch_setup(context):
    ...
    node = Node(
        ...
        # Match the production bucket_bringup names so the dashboard /
        # mission_state_node can drive this single-actuator test rig too.
        name=f"{which}_driver",
