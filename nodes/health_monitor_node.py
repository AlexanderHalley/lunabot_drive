...
EXPECTED_NODES = [
    'drive_node',          # SparkFlex CAN motor driver (src/drive_node.cpp)
    'ekf_filter_node',     # robot_localization EKF
    'apriltag_node',       # apriltag_ros detector
    'apriltag_localizer',  # our map->odom bridge (nodes/apriltag_localizer_node.py)
    'oak',                 # depthai_ros OAK-D driver (basename — namespace stripped)
    'bt_navigator',        # Nav2 behavior tree
    'controller_server',   # Nav2 controller
    'rosbridge_server',    # WebSocket bridge for dashboard
    'actuator_driver',     # bucket actuator (matches both bucket/lift and bucket/tilt)
    'mission_state_node',  # autonomy state machine
]
