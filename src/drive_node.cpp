...
public:
    DriveNode() : Node("drive_node")
    {
        ...
        declare_parameter("log_dir", "/home/lunapi/logs");
        ...
        try {
            ...
            RCLCPP_WARN(get_logger(), "Failed to create log dir %s: %s",
                        log_dir.c_str(), e.what());
        }
        logger_ = std::make_unique<MotorLogger>(log_dir);
        if (logger_->is_open()) {
            RCLCPP_INFO(get_logger(), "Logging to: %s", logger_->filepath().c_str());
        } else {
            RCLCPP_WARN(get_logger(), "Failed to open log file — check %s", log_dir.c_str());
        }
        ...
        // CAN bus status publisher — dashboard subscribes to this for the
        // CAN telemetry cell. We publish a short string summarising motor
        // faults at 1 Hz: "OK" when all four motors are fault-free, or
        // "FAULT m<id>:<hexfaults>" for each faulted motor (sticky bits
        // included). Hardware is healthy if none of the four report faults.
        can_status_pub_ = create_publisher<std_msgs::msg::String>("/can_status", 10);
        can_status_timer_ = create_wall_timer(
            std::chrono::seconds(1),
            std::bind(&DriveNode::publish_can_status, this));
...
private:
    ...
    {
        last_cmd_time_ = now();
        watchdog_tripped_ = false;   // fresh command — clear the warn latch
    ...
    void watchdog_callback()
    {
        ...
        if (elapsed.seconds() > 0.5) {
            // Only warn on the rising edge of a watchdog cutoff so we don't
            // flood the log while the robot is idle and no cmd_vel is being
            // published at all. The flag is cleared in cmd_vel_callback.
            if (!watchdog_tripped_) {
                RCLCPP_WARN(get_logger(),
                            "cmd_vel watchdog — no command for %.2f s, stopping motors",
                            elapsed.seconds());
    ...
    {
        // Aggregate fault state across all four SparkFlex motors. Any
        // non-zero faults / sticky faults → dashboard sees a non-"OK" string.
        struct MotorRef { int id; SparkFlex* m; };
        MotorRef motors[] = {
            {1, left_front_.get()},
            {2, right_front_.get()},
            {3, left_rear_.get()},
        ...
        for (auto& mr : motors) {
            if (!mr.m) continue;
            uint16_t f  = mr.m->GetFaults();
            uint16_t sf = mr.m->GetStickyFaults();
            if (f != 0 || sf != 0) {
                if (any_fault) ss << " ";
                ss << "FAULT m" << mr.id << ":" << std::hex << (f | sf) << std::dec;
                any_fault = true;
            }
        }

        std_msgs::msg::String msg;
        msg.data = any_fault ? ss.str() : std::string("OK");
    ...
    void publish_joint_states()
    {
        ...
        // Bumped from 0.01 to 0.05 so the EKF doesn't over-trust wheel odom
        // when the camera IMU disagrees.
    ...
    void log_callback()
    {
        // NOTE: runs at 10 Hz regardless of motor state. This produces a
        // multi-MB CSV per match even when the robot is idle. If log size
        // becomes a problem, gate on |target_left_| + |target_right_| > 0
        // or compress motor_log_*.csv on shutdown.
    ...
    bool watchdog_tripped_ = false;   // latched warn flag (see watchdog_callback)
